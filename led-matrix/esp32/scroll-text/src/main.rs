...
use max7219_display::led_matrix::scroll::ScrollConfig;
...
async fn main(_spawner: Spawner) {
    ...
    // You can scroll with single device also but will show you 4 daisy-chained devices
    // let mut display = SingleMatrix::from_spi(spi_dev).expect("display count 1 should not panic");

    // Create a display instance for a daisy-chained (Matrix4 means it is for 4 devices, you can use Matrix8 also)
    ...
    let scroll_config = ScrollConfig {
        loop_text: true, // which is default anyway, just as example
        ..Default::default()
    };

    display
        .scroll_text(&mut delay, "impl Rust", scroll_config)
        .unwrap();
}
